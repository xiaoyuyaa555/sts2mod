...
func _add_asset_tree(packer: PCKPacker, asset_root: String, current_dir: String, mod_id: String, manifest_name: String) -> int:
	...
	for file_name in dir.get_files():
		...
		var packed_path := _get_packed_path(mod_id, relative_path)
...
func _get_packed_path(mod_id: String, relative_path: String) -> String:
	if relative_path.begins_with("localization/") or relative_path.begins_with("extracted/") or relative_path.begins_with("gd/"):
		return "res://%s/%s" % [mod_id, relative_path]
