	...
	public static bool IsAiPlayer(Player? player)
	{
		...
		catch (Exception ex)
		{
			if (!_warnedIsAiFailure)
			{
				...
				Log.Warn($"[{ModInfo.Id}][Mayhem][AITeammateCompat] Failed to query AI player identity: {ex.Message}");
			}
	...
	public static string GetDisplayName(Player player)
	{
		...
		catch (Exception ex)
		{
			if (!_warnedDisplayNameFailure)
			{
				...
				Log.Warn($"[{ModInfo.Id}][Mayhem][AITeammateCompat] Failed to query AI player display name: {ex.Message}");
			}
		}
