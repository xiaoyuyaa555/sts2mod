...
internal sealed class PetalActMap : ActMap
{
	...
	private readonly int _specialEliteRow;
	...
	public MapCoord SpecialEliteCoord { get; private set; }
	...
	public PetalActMap(IRunState runState)
	{
		...
		_rng = new Rng(runState.Rng.Seed, "integrated_strategy_petal_map");
		_pointTypeCounts = act.GetMapPointTypes(_rng);
		Grid = new MapPoint[MapWidth, _mapLength];
		_specialEliteRow = GetRowCount() - FixedNodeDistanceFromTop;
	...
	public static bool TryGetSpecialEliteCoord(ActMap map, out MapCoord coord)
	{
		...
		MapCoord specialEliteCoord = new()
		{
			col = col,
			row = row
		};
		MapPoint? point = map.HasPoint(specialEliteCoord) ? map.GetPoint(specialEliteCoord) : null;
		if (point is not { PointType: MapPointType.Elite })
		{
			return false;
		}
	...
	{
		if (_specialEliteRow <= 0 || _specialEliteRow >= GetRowCount())
		{
			throw new InvalidOperationException("Special elite row is out of bounds for PetalActMap.");
		}
		...
		MapPoint specialElite = GetOrCreatePoint(CenterColumn, _specialEliteRow);
		specialElite.PointType = MapPointType.Elite;
		specialElite.CanBeModified = false;
		SpecialEliteCoord = specialElite.coord;
		foreach (MapPoint point in GetPointsInRow(_specialEliteRow).ToList())
		{
			if (point != specialElite)
			{
				RedirectToSpecialElite(point, specialElite);
			}
		}
	...
	private MapCoord GenerateNextCoord(MapPoint current)
	{
		...
		int distanceToSpecialElite = _specialEliteRow - current.coord.row;
		if (distanceToSpecialElite > 3)
		{
			directions.StableShuffle(_rng);
		}
		else if (distanceToSpecialElite > 0)
		{
	...
	{
		int center = GetColumnCount() / 2;
		int distanceToSpecialElite = Math.Abs(row - _specialEliteRow);
		int distanceToTop = _mapLength - 1 - row;
		int upperSpread = Math.Min(center, Math.Max(0, distanceToTop) + 1);
		int spread = Math.Min(center, Math.Min(distanceToSpecialElite, upperSpread));
